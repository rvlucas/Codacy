...
public class GitCliController {
	 ...
	 * @return a response with a JSON that contains the page with a list of
	 *         commits with a maximum of perPage commits
	...
	public Response getCommitList(@QueryParam("url") String url, @QueryParam("page") @DefaultValue("1") int page,
		...
		LOGGER.info("Getting the commits using gitApiService");
		try {
			...
			try {
				LOGGER.warning("An error has occurred while using gitApiService: " + e.getMessage());
				LOGGER.info("Getting the commits using gitCliService");

				gitCliService = new GitCliServiceImpl();
				List<GitCommit> commits = gitCliService.getLogs(url, page, perPage);
				return Response.ok(commits).build();
			} catch (GitCliException e2) {
				LOGGER.severe("An error has occurred while executing: " + e2.getMessage());
				return Response.serverError().entity("An error has occurred while executing: " + e2.getMessage())
						.build();
			}
		}
